...
1. **다운로드**: [GitHub Releases 최신 버전](https://github.com/twbeatles/xteink-crosspoint-websync/releases/latest)에서 실행 파일(`xteink-crosspoint-websync-v1.2.4.exe` 또는 이후 버전)을 다운로드합니다.
...
- **스마트 증분 동기화 (중복 전송 방지)**: SQLite 데이터베이스(`sync_history.db`)를 이용해 이미 기기에 전송한 글의 URL과 해시를 추적합니다. 기기별 이력 모드에서는 각 리더기에 없는 글만 EPUB으로 묶어 전송하므로 여러 기기 사이의 중복 전송도 줄입니다.
...
- **공유 데이터 폴더 지원 (OneDrive / Google Drive / Dropbox)**:
  ...
  - 공유 데이터 가져오기에 실패하면 기사 동기화는 전송 전에 중단합니다. 내보내기에 실패하면 동기화 실패로 표시되므로 공유 폴더 문제를 해결한 뒤 다시 실행하세요.
...
> 🔒 **보안 안내**: 설정 파일(`config.json`) 내의 AI API 키나 서버 인증 토큰은 화면상에서 마스킹(`****`) 처리됩니다. API 키와 토큰은 로컬 설정에만 보관됩니다. 공유 데이터 폴더 동기화는 다른 PC가 같은 리더기를 알아볼 수 있도록 등록된 기기 주소와 이력 식별자를 `synced_posts.json`에 기록하므로, 공유 폴더는 비공개로 유지하세요. 로컬 도서·출력 경로는 내보내지 않습니다.
